...
|     SerialUSB.dtr() method
\*************************************************************/

// Use SFINAE to deal with lack of SerialUSB.dtr() on some platforms
template<class T>
...
void lmic_printf(const char *fmt, ...) {
  ...
  buf[sizeof(buf) - 1] = '\0';
  if (getDtr(Serial)) SerialUSB.print(buf);
}
...
void setup() {
  ...
  while (!SerialUSB); // Wait for Terminal connection before start
  ...
  if (pPinMap == nullptr) {
    ...
    for (;;) {
      ...
      SerialUSB.println(F("board not known to library; add pinmap or update getconfig_thisboard.cpp"));
    }
  }
